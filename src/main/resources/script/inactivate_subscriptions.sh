...
echo "Iniciando processamento de inativação de assinaturas..."
...
for raw_uuid in "${UUID_LIST_RAW[@]}"; do
  ...
  if is_valid_uuid "$clean_uuid"; then
    echo "Processando UUID para inativação: $clean_uuid"
    ...
    if [ $? -eq 0 ]; then
      echo "Assinatura $clean_uuid INATIVADA com sucesso."
    else
      echo "Erro ao inativar assinatura $clean_uuid."
    fi
  else
...
echo "Processamento de inativação de assinaturas concluído."