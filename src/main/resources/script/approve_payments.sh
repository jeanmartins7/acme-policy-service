...
echo "Iniciando processamento de aprovação de pagamentos..."
...
for raw_uuid in "${UUID_LIST_RAW[@]}"; do
  ...
  if is_valid_uuid "$clean_uuid"; then
    echo "Processando UUID para aprovação: $clean_uuid"
    PAYMENT_STATUS="CONFIRMED"
    PRODUCER_COMMAND="echo 'policy-33781e7d:{\"policyId\": \"$clean_uuid\", \"paymentTransactionId\": \"TRANS123456789\", \"status\": \"$PAYMENT_STATUS\", \"amount\": \"100.00\", \"paymentDate\":\"2025-06-10\", \"eventType\": {\"string\": \"PAYMENT_PROCESSED\"}, \"eventSourceId\": {\"string\": \"my-test-source\"}}' | kafka-avro-console-producer --broker-list kafka:9092 --topic payment-confirmed-events-topic --property key.serializer=org.apache.kafka.common.serialization.StringSerializer --property schema.registry.url=http://schema-registry:8081 --property value.subject.name.strategy=io.confluent.kafka.serializers.subject.RecordNameStrategy --property value.schema.file=/tmp/PaymentProcessedEvent.avsc --property parse.key=true --property key.separator=: --property print.key=true --property print.value=true"
    docker exec schema-registry bash -c "$PRODUCER_COMMAND" >/dev/null 2>&1
    if [ $? -eq 0 ]; then
      echo "UUID $clean_uuid aprovado com sucesso."
    else
      echo "Erro ao aprovar UUID $clean_uuid."
    fi
  else
...
echo "Processamento de aprovação concluído."