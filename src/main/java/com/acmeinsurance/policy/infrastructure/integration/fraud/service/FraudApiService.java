package com.acmeinsurance.policy.infrastructure.integration.fraud.service;


import com.acmeinsurance.policy.enums.ClassificationEnum;
import com.acmeinsurance.policy.infrastructure.integration.fraud.client.FraudApiClient;
import com.acmeinsurance.policy.infrastructure.integration.fraud.dto.model.FraudAnalysisResult;
import com.acmeinsurance.policy.infrastructure.integration.fraud.dto.model.Occurrence;
import com.acmeinsurance.policy.infrastructure.integration.fraud.mapper.FraudApiMapper;
...
public class FraudApiService {
    ...
    public Mono<FraudAnalysisResult> analyzePolicyRequest(final UUID orderId, final UUID customerId) {
        ...
        return Mono.fromCallable(() -> fraudApiMapper.toDomain(fraudApiClient.getFraudClassification(orderId, customerId)))
                ...
                .onErrorResume(e -> {
                    log.warn("Fallback: Fraud API failed or returned error for orderId {}. Returning SEM_INFORMACAO.", orderId, e);
                    return Mono.just(FraudAnalysisResult.builder()
                            .orderId(orderId)
                            .customerId(customerId)
                            .analyzedAt(Instant.now())
                            .classification(ClassificationEnum.SEM_INFORMACAO)
                            .occurrences(Collections.singletonList(Occurrence.builder()
                                    .description("Fraud API unavailable or error: " + e.getMessage())
                                    .type(ClassificationEnum.SEM_INFORMACAO.getValue())
                                    .build()))
                            .build());
                });
    }
